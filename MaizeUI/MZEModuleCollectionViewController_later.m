#import "MZEModuleCollectionViewController_later.h"
	...
	if (self) {
		...
		for (NSString *identifier in enabledIdentifiers) {
			if ([_moduleInstanceManager.moduleInstanceByIdentifier objectForKey:identifier]) {
				...
				[orderedSizes addObject:[NSValue valueWithCGSize:CGSizeMake(moduleMetadata.moduleWidth,moduleMetadata.moduleHeight)]];			
			}
		}
...
- (NSArray<MZEModuleInstance *> *)_moduleInstances {
	return [_moduleInstances moduleInstances];
}
